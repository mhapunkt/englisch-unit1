...
const SPECIAL = [
  ...
  ['Welche Schreibweise ist richtig? – Wettbewerb','contest',['kontest','conntest','contast']],
];
...
function makeQuestions(count=20){
  const vocabQs = shuffle(VOCAB).map(([en,de],i)=>{
    ...
    const candidates = shuffle(VOCAB.filter(v=>v[0]!==en)).slice(0,3);
    return askEnglish
      ? {prompt:`Was heißt „${de}“ auf Englisch?`,answer:en,wrong:candidates.map(v=>v[0]),note:`„${de}“ heißt „${en}“.`,kind:'Übersetzung'}
      : {prompt:`Was bedeutet „${en}“?`,answer:de,wrong:candidates.map(v=>v[1]),note:`„${en}“ bedeutet „${de}“.`,kind:'Übersetzung'};
  });
...
function App(){
  ...
  if(done)return <main className="result"><div className="resultMark"><Target/></div><p className="overline">Unit 1 · Bis Part B</p><h1>{score >= 17?'Stark gemacht.':score >= 12?'Gute Runde.':'Noch eine Runde.'}</h1><p className="score"><b>{score}</b> von {questions.length} richtig</p><p>{score>=17?'Die Vokabeln sitzen schon ziemlich sicher.':'Die nächste Runde mischt Fragen und Antworten neu.'}</p><button className="primary" onClick={restart}><RotateCcw/>Neuen Test starten</button></main>;
